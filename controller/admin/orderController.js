...
const getOrderDetails = async (req, res, next) => {
  try {
    ...
    const returnRequest = await Return.findOne({ order: orderId }).populate(
      'items.product',
      'name'
    );
...
const processReturn = async (req, res, next) => {
  try {
    ...
    if (
      ...
      returnRequest.status !== 'Under Review'
    ) {
      return res.status(400).json({
        success: false,
    ...
    if (action === 'approve') {
      ...
      return res.json({
        success: true,
        message:
          'Return approved and refund credited to user wallet successfully.',
