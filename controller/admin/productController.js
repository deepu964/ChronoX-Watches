    ...
    if (req.files && req.files.additionalImages) {
      ...
      for (let file of additionalImageFiles) {
        if (!file.tempFilePath) {
          logger.warn(
            `Skipping invalid additional image: Missing temp file path for ${file?.name || 'unknown'}`
          );
          continue;
        }
...
const editProduct = async (req, res, next) => {
  try {
    ...
    if (name && name !== product.name) {
      const existProduct = await productSchema.findOne({
        name,
        _id: { $ne: editId },
      });
      if (existProduct)
    ...
    if (req.files && req.files.additionalImages) {
      const addImages = Array.isArray(req.files.additionalImages)
        ? req.files.additionalImages
        : [req.files.additionalImages];
