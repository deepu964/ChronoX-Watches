...
const getChangeEmailOtp = (req, res, next) => {
  try {
    if (!req.session.emailChange) {
      return res.redirect(
        '/userProfile?message=Session expired. Please try again.'
      );
    }
...
const verifyChangeEmailOtp = async (req, res, next) => {
  try {
    ...
    if (String(otp).trim() !== String(req.session.userOtp).trim()) {
      return res.redirect(
        '/profile/otp-sent?message=Invalid OTP. Please try again.'
      );
    }
...
const changePassword = async (req, res, next) => {
  try {
    ...
    if (!currentPassword || !newPassword || !confirmPassword) {
      return res
        .status(400)
        .json({ success: false, message: 'No changes to update' });
    }
